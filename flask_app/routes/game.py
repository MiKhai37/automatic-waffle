...
def index():
    ...
    if request.method == 'POST':
        ...
        insert_result = get_mongo_db('games').insert_doc(game_doc)
...
def join_game():
    ...
    game_doc['players'].append(player_doc)
    update_result = get_mongo_db('games').update_one_doc({'id': body['game_id']}, game_doc)
...
def leave_game():
    ...
    player_doc=get_doc_or_404('players', body['player_id'])
    game_doc=get_doc_or_404('games', body['game_id'])
    game_doc['players'].remove(player_doc)
    update_result = get_mongo_db('games').update_one_doc({'id': body['game_id']}, game_doc)
    return Response(response=json.dumps(update_result),
                status=201,
                mimetype='application/json')
