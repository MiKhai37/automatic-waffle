...
def test_insert():
    insert_result = mongo_api.insert_doc(
        {'id': 'test_id', 'test': 'test_value'})
...
def test_update():
    ...
    update_result = mongo_api.update_one_doc(
        {'id': 'test_id'}, {'new_key': 'new_value'})
    assert update_result['Status'] == 'Document Successfully Updated'
    test_info_doc = mongo_api.read_one_doc({'id': 'test_id'})
    assert test_info_doc.get('new_key') is not None

    reupdate_result = mongo_api.update_one_doc(
        {'id': 'test_id'}, {'new_key': 'new_value'})
